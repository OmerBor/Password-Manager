...
class User(object):
    def __init__(self, username, password, salt, info):
    ...
    def __str__(self):
        return "{ Username:"+self.username+", password "+self.password+", salt "+self.salt+" }"
...
class UserDatabase:
    ...
    def saveInfo(self, username, info):
        for user in self.userArray:
            if username == user.username:
             print("set user: "+ username+ " information: "+ user.info)
             user.info = info
        return None
    ...
    def getInfo(self, username):
        for user in self.userArray:
            if username == user.username:
                print("get user: " + username + " information: "+ user.info)
                return user.info

    def hack(self, username):
        for user in self.userArray:
            if username == user.username:
                print("before: "+ user.info)
                user.info=self.manipulate( user.info)
                print("after: " + user.info)


    def manipulate(self, str):
        #changes 3 random letters in string(user info)
        from string import ascii_letters
        inds = [i for i, _ in enumerate(str) if not str.isspace()]
    ...
    def __str__(self):
        return "database with users:\n"+'\n,'.join(map(str,self.userArray))
...
class SimpleWebSocketServer(object):
   def __init__(self, host, port, websocketclass, selectInterval = 0.1):
      ...
      self.clientsMap = collections.ChainMap()
      self.database= UserDatabase()
      self.terminate=False
      #self.database.addUser(User("th","gh0","fg"))
      #print(self.database)
   ...
   def serveforever(self):
       sPass = input("enter password for server\n")
       authK=hashlib.sha256((sPass+"1").encode()).hexdigest()
       encK=hashlib.sha256((sPass+"2").encode()).hexdigest()
       authK=authK[0:16]
       encK = encK[0:16]
       x = threading.Thread(target=self.getInput)
       x.start()
       if(path.exists("serverDB.txt")):
        file = open("serverDB.txt", "r")
        self.myLoad(file, encK, authK )
       while (self.terminate==False):
            self.serveonce()
       self.saveToFile(encK, authK)


   def saveToFile(self, encK, authK):
       file = open("serverDB.txt", "w")
       all_users = [{"username": x.username, "password": x.password, "salt": x.salt, "info": x.info}
                    for x in self.database.userArray]
       serverInfo = json.dumps({"users": all_users})
       cipher = AES.new(str.encode(encK), AES.MODE_CBC)
       ct_bytes = cipher.encrypt(pad(serverInfo.encode(), AES.block_size))
       ct = base64.b64encode(ct_bytes).decode('utf-8')
       iv = base64.b64encode(cipher.iv).decode('utf-8')
       msg = str.encode(iv + ct)
       msgSTR = base64.b64encode(msg).decode('utf-8')
       h = HMAC.new(authK.encode(), digestmod=SHA256)
       h.update(msg)
       mac = h.hexdigest()
       result = json.dumps({'mac': mac, 'msg': msgSTR, 'iv': iv, 'ct': ct})
       file.write(result)
       file.close()
   ...
   def myLoad(self, file, encK, authK):
            #load database from file
            b64 = json.load(file)
            mac = b64["mac"]
            msg = base64.b64decode(b64["msg"])
            h = HMAC.new(authK.encode(), digestmod=SHA256)
            h.update(msg)
            try:
               h.hexverify(mac)
            ...
            try:
                ...
                cipher = AES.new(str.encode(encK), AES.MODE_CBC, iv)
                pt = unpad(cipher.decrypt(ct), AES.block_size)
                print("The message was: ", pt, "\n")
                pt = pt.decode('utf-8')
                js = json.loads(pt)
                arr = js["users"]
                arr = list(map(lambda x: User(x["username"], x["password"], x["salt"], x["info"]), arr))
                print(arr)
                print(arr[0].info)
                self.database.userArray = arr
            except ValueError:
